...
def objective(trial):
    ...
    hidden_units = trial.suggest_int('n_units', low=1, high=16, step=1)  # single value for all layers
    ...
    loss_shear = tf.reduce_mean(tf.abs((y_p[:,0] - tf.reshape(y, [batch_size, 2])[:,0])/tf.reshape(y,[batch_size,2])[:,0]))
    loss_velo = tf.reduce_mean(tf.abs((y_p[:,1] - tf.reshape(y, [batch_size, 2])[:,1])/tf.reshape(y,[batch_size,2])[:,1]))
    ...
    validation_loss_shear = np.mean(np.abs((y_valid_pred[:,0] - y_Veri_scaled[:,0])/y_Veri_scaled[:,0]))
    validation_loss_velo = np.mean(np.abs((y_valid_pred[:,1] - y_Veri_scaled[:,1])/y_Veri_scaled[:,1]))
  
    validation_loss = (validation_loss_shear + validation_loss_velo ) / 2
    ...
    if validation_loss <= objective.best_val_loss:
       objective.best_val_loss_shear = validation_loss_shear
       objective.best_val_loss_velo = validation_loss_velo
       objective.best_val_loss = validation_loss
                                                                                                                                                                                                                                                                                                                                                                                           
  
    
    print("\n")
    print(f"Trial {trial.number} - Loss Shear: {validation_loss_shear:.7f}, Loss Velo: {validation_loss_velo:.7f}")
    ...
    #print("\n")
    print(f"Best Validation Losses: Shear={objective.best_val_loss_shear:.7f}, Velo={objective.best_val_loss_velo:.7f}")
